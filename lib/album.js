...
exports.fetch = function(album_url, callback) {
	log.log('pvlist', 'fetch: starting')
	var w = pw.Page({
		contentURL: album_url,
		contentScriptFile: data.url('content/engine/pvlworker.js')
	});
	w.port.on('pvlist', function(pvs) {
		w.destroy();
		log.log('pvlist', 'fetch: found',
			pvs.length, 'photos or videos');
		callback(pvs);
	});
	w.port.on('log', log.log.bind(null, 'pvlist', 'DEBUG:'));
};
